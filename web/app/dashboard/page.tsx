...
function BusinessInventoryValuePanel({
  ...
  return (
    <div className="overflow-hidden rounded-md border border-slate-200">
      <div className="border-b border-slate-200 bg-slate-50 px-3 py-2">
        <div className="text-sm font-semibold">Business Inventory And Cash Value</div>
        <div className="text-xs text-slate-500">Inventory value plus Amazon and YNAB cash balances</div>
      </div>
...
function ValueRow({
  ...
  const content = (
    <>
      <div>
        <div className={emphasis ? "font-semibold text-slate-900" : "text-slate-700"}>
          {label}
        </div>
        {detail ? <div className="text-xs text-slate-500">{detail}</div> : null}
      </div>
      <div className={`whitespace-nowrap text-right font-semibold ${emphasis ? "text-slate-950" : ""}`}>
        {loading ? "--" : value}
      </div>
    </>
  );
    ...
    return (
      <button
        ...
        className={`flex w-full items-start justify-between gap-3 px-3 py-2 text-left hover:bg-slate-50 ${
          emphasis ? "bg-slate-100 cursor-pointer" : ""
        }`}
      >
        {content}
      </button>
