...
- Prep pricing compares the stored sell price with latest Amazon sales, Keepa price intelligence, the operator's current Amazon listing price/quantity when in stock, and cached Amazon Product Fees estimates before shipment creation.
...
- Pricing columns show Buy Price, editable Sell Price, My Price/in-stock quantity, Last Sold Price/date, Current Price, Keepa 90-day Buy Box average, Amazon estimated fees, and net Profit/ROI against Buy Price.
- My Price is read from current `amazon_skus` rows for the ASIN, with quantity filled from the latest FBA inventory snapshot when Amazon-held/inbound inventory exists. It shows the lowest active in-stock listing price and total Amazon-held/inbound units.
- Current Price is sourced only from Keepa. The lightweight stats snapshot supports current Buy Box, current FBA new price, and current New price. A generic current New fallback is displayed without an FBA/MF icon because the lightweight call does not prove whether that offer is merchant-fulfilled. Precise lowest-MF classification requires offer-level Keepa data.
...
- Sell Price is visually flagged when it is below Last Sold, Current Price,
