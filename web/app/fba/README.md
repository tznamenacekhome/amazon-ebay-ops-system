...
- My Price is read from current `amazon_skus` rows for the ASIN, with quantity filled from the latest FBA inventory snapshot when Amazon-held/inbound inventory exists. It shows the lowest active in-stock listing price and total Amazon-held/inbound units; fulfillment icons mark FBA/MF and whether the displayed price matches the current Buy Box.
- Current Price is sourced only from Keepa. It falls back in this order: current Buy Box, current FBA new price, current merchant/new price, then stacked `Used Only` when no new-offer price is available. Fulfillment icons mark the Keepa price source and Buy Box state.
