...
let positive=true, missingEvidence=false;const reads=[];
...
assert.equal(row.diagnosticComparison.rows.find(r=>r.key==='ebay_description').ebay,'Original full description','Visible evidence survives compact qualification');
assert(reads.some(q=>q.ids&&q.select.includes('display_price:raw_ebay_json->price')));
assert(reads.some(q=>q.ids?.length===1&&q.select==='opportunity_id,sourcing_ebay_candidates(raw_ebay_json)'),'Full raw evidence is fetched only for visible IDs');
missingEvidence=true;
const changed=await route.GET({url:'https://example.test/api/sourcing/opportunities?status=business_excluded&limit=50'});
assert.equal(changed.status,500);assert.match(changed.body.error,/changed while loading/);
missingEvidence=false;positive=false;reads.length=0;const unknown=await route.GET({url:'https://example.test/api/sourcing/opportunities?status=business_excluded&limit=50'});
