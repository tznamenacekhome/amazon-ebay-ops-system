...
latest=await save({pairVerdict:'incorrect',corrections:[]},{identityAttested:false,variationVerified:false});assert.equal(latest.pairVerdict,'incorrect');assert(!latest.tierA);
...
latest=await save({pairVerdict:'correct',corrections:[]},{identityAttested:true,variationVerified:false});assert(!latest.tierA,'Unverified variation scope cannot qualify a positive as Tier A');
...
latest=await save({pairVerdict:'correct',flaggedFields:['coreGame'],corrections:[c]},{identityAttested:true,variationVerified:true});assert(latest.tierA);assert.equal(latest.corrections[0].value,'Corrected game');assert(latest.corrections[0].before);
...
failReadback=true;const readbackBody={...body,requestId:randomUUID(),expectedRevision:(await service.loadState(row)).revision,identityAttested:false,variationVerified:false,feedback:{pairVerdict:'incorrect',corrections:[]}};const readbackResult=await post(readbackBody);failReadback=false;
