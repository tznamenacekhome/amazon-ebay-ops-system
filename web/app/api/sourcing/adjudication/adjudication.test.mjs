...
latest=await save({pairVerdict:'not_provided',flaggedFields:['coreGame'],corrections:[{...c,value:'Later correction'}]});assert.equal(latest.pairVerdict,'correct');assert(!latest.tierA,'New evidence after confirmation requires re-review');assert.equal(lastSave.p_context.variationResolution,undefined);assert.equal(lastSave.p_context.variationVerified,undefined);assert.equal(lastSave.p_context.variationScopeReviewed,undefined);assert.equal(latest.corrections[0].value,'Later correction');
