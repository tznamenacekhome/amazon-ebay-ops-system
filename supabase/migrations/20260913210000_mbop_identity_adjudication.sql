-- MBOP: isolated frozen-pair adjudication in existing v3 evidence tables.
-- No opportunity, lifecycle, purchase, hold or historical snapshot rewrites.
-- Serialize normal review writes with adjudications; exclude adjudications from live routing reads.
create or replace function public.sourcing_save_review(
...
create or replace function public.sourcing_latest_reviews(p_pairs jsonb) returns jsonb
language sql stable security invoker set search_path='' as $$
  ...
  left join lateral (select action_id,raw_action_context,created_at from public.sourcing_actions
    where asin=p.asin and ebay_item_id=p.ebay_item_id
      ...
      and coalesce(raw_action_context->>'source','') <> 'identity_adjudication_queue'
  ...
  left join lateral (
    select jsonb_agg(correction) corrections from (
      ...
      where r.asin=p.asin and r.raw_action_context->'matchingFeedback'->>'version'='matching_feedback_v3'
        and coalesce(r.raw_action_context->>'source','') <> 'identity_adjudication_queue'
...
create function public.sourcing_adjudication_state(p_asin text, p_ebay_item_id text)
returns jsonb language plpgsql stable security invoker set search_path='' as $$
declare rows jsonb; revision text;
begin
  select jsonb_agg(to_jsonb(x) order by x.created_at desc,x.action_id desc) into rows from (
    select action_id,asin,ebay_item_id,created_at,listing_snapshot_id,raw_action_context
    from public.sourcing_actions
    where asin=p_asin and raw_action_context->'matchingFeedback'->>'version'='matching_feedback_v3'
      and (ebay_item_id=p_ebay_item_id or (
        split_part(regexp_replace(ebay_item_id,'^v1[|]',''),'|',1)=split_part(regexp_replace(p_ebay_item_id,'^v1[|]',''),'|',1)
        and (ebay_item_id not like 'v1|%|%' or p_ebay_item_id not like 'v1|%|%')) or exists (
        select 1 from jsonb_array_elements(coalesce(raw_action_context->'matchingFeedback'->'corrections','[]'::jsonb)) c
        where c->>'side'='amazon' and c->>'scope'='asin'))
    order by created_at desc,action_id desc limit 1001
  ) x;
  if jsonb_array_length(coalesce(rows,'[]'))>1000 then
    raise exception 'Review history exceeds bounded scope; archive-aware review required' using errcode='54000';
  end if;
  revision:=md5(coalesce(rows,'[]')::text);
  return jsonb_build_object('revision',revision,'actions',coalesce(rows,'[]'));
end $$;
revoke all on function public.sourcing_adjudication_state(text,text) from public,anon,authenticated;
grant execute on function public.sourcing_adjudication_state(text,text) to service_role;

create function public.sourcing_save_adjudication(
  p_request_id uuid,p_asin text,p_ebay_item_id text,p_expected_revision text,
  p_actor text,p_request_hash text,p_context jsonb,p_snapshot jsonb
) returns jsonb language plpgsql security invoker set search_path='' as $$
declare prior public.sourcing_actions%rowtype; snap public.sourcing_listing_snapshots%rowtype;
  context jsonb; snapshot_id uuid:=gen_random_uuid(); recorded_at timestamptz:=clock_timestamp();
  verdict text; label text; label_type text; op_id uuid;
begin
  if p_request_id is null or nullif(p_actor,'') is null or nullif(p_request_hash,'') is null
    or nullif(p_asin,'') is null or nullif(p_ebay_item_id,'') is null
    or p_context->>'source' is distinct from 'identity_adjudication_queue'
    or p_context->>'snapshotPolicy' is distinct from 'frozen_historical'
    or nullif(p_context->>'queueSnapshotHash','') is null
    or p_context->'pair'->>'asin' is distinct from p_asin
    or p_context->'pair'->>'ebayItemId' is distinct from p_ebay_item_id
    or p_snapshot->>'asin' is distinct from p_asin
    or p_snapshot->>'ebay_item_id' is distinct from p_ebay_item_id then
    raise exception 'Invalid frozen exact-pair adjudication' using errcode='22023';
  end if;
  verdict:=p_context->'matchingFeedback'->>'pairVerdict';
  if p_context->'matchingFeedback'->>'version' is distinct from 'matching_feedback_v3'
    or verdict is null or verdict not in ('correct','incorrect','unsure','not_provided') then
    raise exception 'Explicit v3 review required' using errcode='22023';
  end if;
  perform pg_advisory_xact_lock(hashtextextended(p_request_id::text,0));
  select * into prior from public.sourcing_actions where action_id=p_request_id;
  if found then
    if prior.raw_action_context->>'requestHash' is distinct from p_request_hash
      or prior.raw_action_context->>'actor' is distinct from p_actor
      or prior.asin is distinct from p_asin or prior.ebay_item_id is distinct from p_ebay_item_id then
      raise exception 'Request ID reused with different evidence' using errcode='22023';
    end if;
    return jsonb_build_object('actionId',prior.action_id,'snapshotId',prior.listing_snapshot_id,'replayed',true);
  end if;
  perform pg_advisory_xact_lock(hashtextextended('identity-review-asin|'||p_asin,0));
  if public.sourcing_adjudication_state(p_asin,p_ebay_item_id)->>'revision' is distinct from p_expected_revision then
    raise exception 'A newer pair review or ASIN correction exists. Reload before saving.' using errcode='40001';
  end if;
  if verdict='correct' and p_context->>'identityAttested' is distinct from 'true' then
    raise exception 'Confirm Match requires explicit exact identity attestation' using errcode='22023';
  end if;
  recorded_at:=clock_timestamp();
  op_id:=nullif(p_context->'pair'->>'opportunityId','')::uuid;
  context:=p_context||jsonb_build_object('actor',p_actor,'recordedAt',recorded_at,'requestHash',p_request_hash,
    'requestId',p_request_id,'snapshotId',snapshot_id,'admissionUse','evidence_only');
  insert into public.sourcing_actions(action_id,opportunity_id,asin,ebay_item_id,action_type,notes,raw_action_context,created_at)
    values(p_request_id,op_id,p_asin,p_ebay_item_id,'matching_feedback',p_context->>'notes',context,recorded_at);
  snap:=jsonb_populate_record(null::public.sourcing_listing_snapshots,p_snapshot||jsonb_build_object(
    'listing_snapshot_id',snapshot_id,'action_id',p_request_id,'opportunity_id',op_id,
    'snapshot_event','matching_feedback','snapshot_source','identity_adjudication_queue',
    'raw_context_json',context,'captured_at',recorded_at,'created_at',recorded_at));
  insert into public.sourcing_listing_snapshots select snap.*;
  update public.sourcing_actions set listing_snapshot_id=snapshot_id where action_id=p_request_id;
  label:=case verdict when 'correct' then 'match' when 'incorrect' then 'non_match' else 'needs_review' end;
  label_type:=case verdict when 'correct' then 'positive_identity' when 'incorrect' then 'negative_identity' else 'unknown' end;
  insert into public.matching_intelligence_examples(source_table,source_id,source_detail,listing_snapshot_id,
    opportunity_id,action_id,asin,ebay_item_id,amazon_title,ebay_title,match_label,label_type,
    evidence_strength,raw_context_json,reviewed_at)
  values('sourcing_actions',p_request_id::text,'matching_feedback',snapshot_id,op_id,p_request_id,p_asin,
    p_ebay_item_id,snap.amazon_title,snap.ebay_title,label,label_type,'high',context,recorded_at);
  return jsonb_build_object('actionId',p_request_id,'snapshotId',snapshot_id,'replayed',false);
end $$;
revoke all on function public.sourcing_save_adjudication(uuid,text,text,text,text,text,jsonb,jsonb) from public,anon,authenticated;
grant execute on function public.sourcing_save_adjudication(uuid,text,text,text,text,text,jsonb,jsonb) to service_role;
