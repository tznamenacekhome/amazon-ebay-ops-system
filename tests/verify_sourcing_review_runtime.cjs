...
// Evaluate rolling sales windows at the frozen evidence capture time.
const NativeDate=Date, frozenNow=NativeDate.parse(baseline.startedAt);assert(Number.isFinite(frozenNow));
global.Date=class extends NativeDate {constructor(...args){super(...(args.length?args:[frozenNow]));}static now(){return frozenNow;}static parse(v){return NativeDate.parse(v);}static UTC(...args){return NativeDate.UTC(...args);}};
...
for(const file of fs.readdirSync(path.join(app,'.next/server'),{recursive:true}).filter(f=>f.endsWith('.js'))){const s=fs.readFileSync(path.join(app,'.next/server',file),'utf8');// A frozen evaluation version is data, not executable policy activation.
const executable=s.replaceAll('video_game_identity_phase3_shadow_v3','FROZEN_EVALUATION_VERSION');assert(!executable.includes('phase3_shadow')&&!executable.includes('offline_identity_policy'),file);}
