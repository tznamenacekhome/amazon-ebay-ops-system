...
## Positive safety evidence hierarchy — 2026-09-13

Historical workflow-positive records are not automatically exact-pair ground truth. Receiving defaults, Purchased actions, automatic purchase links, RevSeller enrichment and reusable manual title memory cannot independently certify exact retail identity. Only current explicit scoped pair review with reconciled source/correction provenance can enter Tier A. An empty Tier A corpus is not a passing gate. Field defects, input-coverage losses and physical delivery errors remain separate; prior wrong-ASIN negatives do not transfer to corrected pairs. See [the reconciliation](docs/sourcing_positive_evidence_reconciliation_2026-09-13.md). No production behavior was changed by this decision.
