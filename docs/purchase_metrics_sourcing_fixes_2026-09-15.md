...
### Initial local build and approval checkpoint (superseded below)

Source commit `6db194f82a53` was pushed to origin/main. The production Docker build, including Next.js compile and TypeScript checks, passed locally as `mbop-web:ops-20260915` (build identifier `6db194f`, manifest-list digest `sha256:57620f363bd693e3772950f2b24420f563d2f648e762d7f81a49cb2d3c8a97ca`). This initial local image was not pushed to ECR or deployed. Existing dependency audit findings were emitted during npm ci; dependencies were unchanged.

Automatic approval review rejected `scripts/deploy-web.ps1 -NoWait` before execution because the latest request authorized implementation/investigation without explicit authorization to deploy these new changes. No workaround attempted. Deployment approval is required. The additive purchase RPC was already applied and verified before this rejection. Production ECS readback confirms unchanged `mbop-web-task:154`, one running task, zero pending and rollout COMPLETED. On approval, use the normal deploy-web/status workflow, then browser verification if a session is available; do not combine this web release with a sourcing rerun or capacity change.

### Authorized production release

The operator subsequently explicitly authorized web deployment. Deployed `mbop-web-task:155` from commit `8506bd4c48dc` (application change `6db194f82a53`); image `sha256:c33af08bc7fb67259f6feb0858b1aa6eb39868fab5f0fcf29a4f2f8e5057288c`. Production Docker build and TypeScript passed. AWS services-stable passed, rollout is COMPLETED, desired/running 1 and pending 0; the exact running task's load-balancer target is healthy. Container configuration matches web154 except image/build identifiers. Verification: `tmp/ops-20260915/web155-deployment.json`. Browser inventory remains empty, so authenticated click-through is not claimed. No scheduler changes or sourcing rerun.

The requested read-only [complete storage report](database_storage_report_2026-09-15.md) and [exact-byte inventory](database_storage_inventory_2026-09-15.json) cover both Supabase projects, every table and cleanup candidates. No cleanup or resize was performed.
