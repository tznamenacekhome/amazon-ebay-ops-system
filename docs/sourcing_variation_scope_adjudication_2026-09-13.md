...
## Completed web-only release

Implementation `a0a466d`; exact UTF-8 label fix/release `7de113fec445`; both pushed. Web150 is stable with one running task and a COMPLETED rollout; exact target `172.31.16.207:3103` is healthy. Image: `297464765814.dkr.ecr.us-west-2.amazonaws.com/mbop-web@sha256:d039f7a2118778f1c2a5e90d6f2cfcd36f6d8473987a372f2be7445cff97bf83`. The exact release image passes the browser and networking-disabled routing tests. Scheduler92's full task definition and all 20 schedule configurations/targets are unchanged. No scheduler deployment occurred. CLOUD_DEPLOYMENT=true and LOCAL_SYNC_ENABLED=false remain set.

Bounded production readback used 32 state RPC reads and confirms 0 of 12 variation scopes reviewed: Tier A 0, unqualified confirmations 12, verified negatives 3, Not Sure 0, informational 1. All fifteen original verdict action IDs and all seventeen correction counts are preserved. There is no new operator follow-up to replay yet. The disposable database is stopped. No sourcing refresh, provider search, marketplace write, routing/hold/lifecycle mutation or synthetic production review occurred. No browser was available for authenticated production verification; this limitation remains explicit.

See [release proof](sourcing_variation_scope_deployment_2026-09-13.json). Next action belongs to the operator: use the filtered confirmation view, select variation scope, and Save variation scope for each of the twelve rows. After that real evidence exists, resume the bounded corpus capture and strict gate; do not activate Phase 3.

Deployment tooling note: the shell wrapper returned exit code 1 although the script log ended `Deploy complete`. Independent AWS status and exact task/ALB readback confirmed the completed, healthy web150 rollout. The release conclusion relies on those readbacks.
